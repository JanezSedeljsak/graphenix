...
\DeclareFloatingEnvironment[fileext=loc,placement={!ht},name=Koda]{code}
\renewcommand\listingscaption{Koda \thecode:}
...
\chapter{Uvod}
    ...
    \begin{figure}[h]
        \centerline{\includegraphics[width=1\textwidth, angle=0]{inf_arhitektura.png}}
...
\end{minted}
% \label{er_model_def}
% \caption{Definicija modelov za predstavitev funkcionalnosti}
% \end{code}
...
\end{minted}
        ...
        Ob naboru podatkov izvedemo pristop od zgoraj navzdol, saj najprej preberemo podatke za korensko entiteto in se nato spuščamo v nižje nivoje.

        Vsako vozlišče drevesa iz slike \ref{deep_nested_query} predstavlja ločeno branje matrične datoteke podatkov. Tekom nabora tako dvakrat izvedemo branje datoteke s podatki ``nalog`` ({\tt Task}). Pristop zgornjega nabora v klasičnih SQL poizvedbah ni mogoč, saj dobimo kartezičen produkt in moramo rezultate potem še naknadno obdelati.
        ...
        \newline
        \noindent
        Ko govorimo o agregaciji imamo v mislih poizvedbe, kjer iščemo različne statistične rezultate iz posamezne tabele. MySQL DBMS podpira kar nekaj različnih operaciji za agregacijo \cite{MYSQL_AGG} ({\tt AVG, COUNT, MAX, MIN, SUM, STD, STDDEV, VARIANCE} ...).
