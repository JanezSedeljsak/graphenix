...
\end{minted}
    ...
    Na sliki \ref{graphenix_structure} je diagram, ki prikazuje strukturo celotnega sistema z vsemi pomembnimi komponentami in orodji, ki omogočajo komunikacijo med njimi.
    ...
    Jedro vsebuje vse performančno gledano kritične operacije. Jedro za shranjevanje vseh podatkov uporablja binarne datoteke, ki so zunanja komponenta knjižnice, saj služijo le za shranjevanje podatkov.
   ...
   V našem primeru se ob vsaki posodobitvi kode izvede akcija za preverjanje pravilnega delovanja aplikacije. Akcije se izvedejo za dve verziji programskega jezika Python (v3.10 in v3.11), postavitev pa se izvede na virtualnem Linux okolju.
...
\label{ch2}
   ...
   \begin{figure}[H]
    ...
    V temnem delu stolpca je prikazana razlika velikosti z in brez uporabe indeksne strukture. DBMS z najnižjo porabo prostora je SQLite. Razlog za nižjo porabo se nahaja v velikosti posameznega vozlišča in samem algoritmu vstavljanja v drevo, ki lahko vpliva na število nivojev v drevesu.
...
\end{minted}
    ...
    \noindent
    Iz grafa je jasno razvidno, da uporaba ORM ni dobro pripravljena za masovno nalaganje podatkov, saj se ponovno kreirajo instance, ki predstavljajo, več kot 70\% delež celotne poizvedbe. V primeru MySQL se ponovno pojavi tudi nekaj dodatne zakasnitve zaradi načina prenosa podatkov preko TCP/IP protokola.
...
\end{minted}
    ...
    \noindent
    Sam čas izvajanja posameznega branja je relativno nizek, v vseh primerih je bilo branje izvedeno v manj kot 100 ms. Opazimo, da omejitve ORM tukaj ne pridejo toliko do izraza, saj je potrebno kreirati le 500 instanc razredov. Pride pa do izraza zakasnitev, ki jo za prenos podatkov prinaša TCP/IP protokol, kar je videno v primerih uporabe MySQL.
...
\end{minted}
    ...
    \noindent
    Ponovno se izkaže, da ORM s seboj prinese dodatne zakasnitve zaradi kreiranja instanc. Za razliko od prejšnjih scenarijev je rezultat v tem primeru drugačen v primeru uporabe ORM in uporaba klasične poizvedbe.
    \begin{itemize}
...
\end{minted}
    ...
    Rezultati sicer dobro sledijo trendom iz predhodnih scenarijev – MySQL zaradi načina prenosa podatkov potrebuje nekoliko več časa, poleg tega pa se ponovno pozna razlika uporabe ORM in uporaba SQL poizvedb. Graphenix, pa je tudi tukaj primerljiv uporabi SQLite brez uporabe ORM.
...
\end{minted}
    ...
    Kaj hitro smo prišli do ugotovitve, da težava ni bil proces iskanja v B drevesu, temveč je bila težava branje zaglavja datoteke entitete. Metoda za branje podatkov iz posamezne entitete je delovala, tako da je najprej prebrala celotno datoteko zaglavja entitete in za posamezen identifikator zapisa pridobila še odmik v matriki zapisov. Izkazalo se je, da je ta proces zavzel nekje $\approx 85$\% izvajalnega časa.

    V namen hitrejšega izvajanja se branja celotnega zaglavja izognemu v primeru, da imamo že vnaprej določene identifikatorje, ki jih iščemo in je teh manj kot \textbf{IX\_THRESHOLD}, kar predstavlja mejo branja vsakega zapisa ločeno in je v Graphenix nastavljen na vrednost \num{100}.
    ...
    \noindent
    Iz grafa razberemo, da v vseh treh izvedbah zapis najdemo v manj kot 2 ms. Nekaj zakasnitve se sicer vseeno pojavi iz strani MySQL, kar je glede na predhodne scenarije pričakovano.

    Poleg tega v vseh treh primerih, dobimo več \num{100}x pohitritev v primerjavi z branjem brez uporabe indeksa.
...
\end{minted}
    ...
    V tem segmentu je predstavljen razvoje preproste namizne aplikacije za beleženje stroškov. Aplikacija omogoča pregled stroškov, vnašanje, urejanje in brisanje, ter statističen pregled, kjer so stroški grupirani glede na kategorije.
    ...
    \noindent
    Za vsak posamezen strošek sta definirani akciji brisanja in urejanja. Poleg tega lahko stroške glede na naziv, ceno in kategorijo tudi urejamo. Hkrati, pa je stroške mogoče tudi filtrirati po nazivu.
    ...
    Grahepnix je dobra izbira, saj gre za relativno nezahtevno aplikacijo, kjer ne potrebujemo skrbeti za več hkratnih dostop do baze podatkov, poleg tega pripravljen ORM omogoča tudi bistveno lažjo manipulacijo podatkov.
...
\chapter{Sklepne ugotovitve}
    ...
    \begin{enumerate}
        ...
        Kljub temu, pa ostaja še ogromno scenarijev, ki jih z našo implementacijo ne dosežemo. Sploh je to vidno v množičnih akcijah, kot npr. hkratno iskanje več zapisov in množično vnašanje v B drevo.
        ...
        \begin{itemize}
            \item SQLite nize definira s strukturo ``TEXT``, ki besedilo shranjuje v ločeni strukturi od zapisov in po potrebi zapise v tej ločeni strukturi premika.
    ...
    Razvoj dobrega DBMS ni trivialen, kar je mogoče razpoznati že iz tega, da se produkcijsko dokazani DBMS sistemi konstantno posodabljajo in odpravljajo manjše napake, ki se še vedno pojavljajo.
    ...
    Ključnega pomena tekom razvoja je bila tudi implementacija indeksiranja z uporabo B drevesa. S tem smo uporabnikom namenske knjižnice omogočili možnost določanja indeksov, ki bistveno pospešijo iskanje. Gre definitivno za najbolj kompleksen del celotnega sistema, saj je sami implementaciji drevesa z uporabo C++ kazalcev dodana še kompleksnost shranjevanja v datoteki. Posledično ostaja veliko funkcionalnosti, ki znotraj segmenta indeksiranja niso dobro ali sploh niso implementirane.
    
    Poleg tega smo tekom razvoja dodali še lasten ORM, ki direktno komunicira z operacijami jedra, kar bistveno zmanjša kompleksnost, ki jo sicer prinese ORM, kjer moramo skrbeti še za dodatne preslikave med modeli in SQL poizvedbami. Veliko operacij je prilagojenih objektnemu pristopu branja, kar se razlikuje od klasičnih relacijskih podatkovnih baz in se nekoliko približa pristopu NoSQL.
    
    Kot celota je bil razvoj uspešen, z dodatkom različnih testnih scenarijev smo tekom razvoja zagotovili pravilno delovanje. Analiza uspešnosti je pokazala primerljive rezultate z MySQL in SQLite DBMS. Za konec, pa smo izdelali še nekaj preprostih primerov uporabe namenske knjižnice Grahepnix.
