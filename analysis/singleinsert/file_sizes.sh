...
if [ -d "graphenix_db/singleinsert_$size" ]; then
    echo "  Found graphenix db..."
    graphenix_size=$(du -sb graphenix_db/singleinsert_$size | awk '{print $1}')
    echo "$graphenix_size" >> "analysis/singleinsert/size_$size.txt"
    echo "  Size was: $graphenix_size"
fi

if [ -f "graphenix_db/sqlite_singleinsert_alchemy_$size.db" ]; then
    echo "  Found sqlite db..."
    sqlite_size=$(du -sb graphenix_db/sqlite_singleinsert_alchemy_$size.db | awk '{print $1}')
...
mysql_size=$(docker exec -it fdc7ff9ad883 bash -c "mysql -u root -proot << EOF
    ...
    FROM tables as t
    WHERE t.table_schema = 'singleselect_200000';
EOF
